...
--  Ensuite 20 clics a une par seconde, StartDungeon, puis RECHARGEMENT
--  COMPLET de kronax_new.lua.
--
--  Ce rechargement est le point cle. Sans lui, c'est la meme instance de
--  Kronax qui continue apres le retry : elle reste accrochee a l'ancien
--  boss, ses drapeaux "une seule fois" sont deja consommes, son H initial
--  aussi. Elle tourne mais ne sert a rien. En la rechargeant, la nouvelle
--  run repart sur un script neuf, qui renvoie son H et se raccroche au
--  nouveau boss tout seul.
-- ==========================================================
...
--  Rechargement du farm pour la nouvelle run.
local RELOAD_KRONAX = true
local REPO = "https://raw.githubusercontent.com/rayzox-akpl/teste2/main/"
local KRONAX_FILE = "kronax_new.lua"
local RELOAD_DELAY = 3     -- apres StartDungeon, laisse la run demarrer
...
local function retryDungeon20Times()
    ...
    -- On laisse la nouvelle run demarrer, puis on recharge le farm.
    task.wait(RELOAD_DELAY)
    retryRunning = false

    if not RELOAD_KRONAX then return end
    if not actif() then return end

    --  Le farm refuse desormais tout chargement en double : il faut donc
    --  l'arreter d'abord, et attendre sa confirmation. Il remet
    --  _G.StopCurrentFarm a nil quand il a fini.
    if typeof(_G.StopCurrentFarm) == "function" then
        print("[RETRY] arret de l'instance en cours avant rechargement")
        pcall(_G.StopCurrentFarm, "rechargement apres retry")

        local limite = os.clock() + 5
        while typeof(_G.StopCurrentFarm) == "function"
            and os.clock() < limite do
            task.wait(0.1)
        end

        if typeof(_G.StopCurrentFarm) == "function" then
            warn("[RETRY] arret non confirme -- rechargement abandonne")
            warn("[RETRY] l'instance en place continue, aucun doublon cree")
            return
        end
        print("[RETRY] instance arretee, place libre")
    end

    local url = REPO .. KRONAX_FILE .. "?v=" .. tostring(tick())
    ...
    if not ok or type(source) ~= "string" or #source < 40
        ...
        warn("[RETRY] rechargement impossible : " .. url)
        return
    end
    local fn, err = loadstring(source, "@KronaxNew")
    if not fn then
        warn("[RETRY] compilation : " .. tostring(err))
        return
    end
    print("[RETRY] rechargement de " .. KRONAX_FILE
        .. " pour la nouvelle run")
    -- Derniere instruction : ce chargement va remplacer cette instance
    -- elle-meme, via les jetons. Tout ce qui suivrait serait ignore.
    pcall(fn)
end
