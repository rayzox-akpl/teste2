...
do
    --  Ces deux constantes sont volontairement DANS ce bloc et non au
    --  niveau superieur : Luau limite un chunk a 200 variables locales, et
    --  ton script est deja tout pres du plafond. Un local de bloc libere
    --  son registre a la fin du bloc, un local de niveau superieur non.
    local REPO = "https://raw.githubusercontent.com/rayzox-akpl/teste2/main/"
    local DELAI_AUTO_START = 9   -- decalage avant le chargement d'auto_start
    ...
    task.spawn(function()
        ...
        if _G.AutoStartDone then
            ...
            --  Trois secondes de decalage : le farm a le temps de finir son
            --  initialisation, de se brancher au boss et de poser son
            --  noclip avant que la cinematique soit sautee et le donjon
            --  lance. Charge trop tot, il manquait le debut de la run.
            task.wait(DELAI_AUTO_START)
            print("[kronax] " .. DELAI_AUTO_START
                .. " s ecoulees, chargement d'auto_start")
    ...
    --  ARRET GLOBAL SUR B
    --
    --  Le farm a deja son propre B, mais il ne coupe que lui-meme :
    --  auto_start et auto_retry continuaient de tourner, et auto_retry
    --  finissait par recharger un farm que tu venais d'arreter. On ajoute
    --  donc un second ecouteur qui coupe les trois.
    --
    --  On garde une reference dans _G pour pouvoir la debrancher si le
    --  script est recharge, sinon les ecouteurs s'accumuleraient.
    if _G.KronaxArretGlobal then
        pcall(function() _G.KronaxArretGlobal:Disconnect() end)
    end

    _G.KronaxArretGlobal = game:GetService("UserInputService").InputBegan
        :Connect(function(input, processed)
            if processed then return end
            if input.KeyCode ~= Enum.KeyCode.B then return end

            print("[kronax] B -- arret de TOUS les scripts")
            --  auto_retry en premier : sinon il verrait le farm s'arreter,
            --  croirait a une fin de run et rechargerait aussitot.
            for _, nom in ipairs({"StopAutoRetry", "StopAutoStart",
                "StopCurrentFarm"}) do
                if typeof(_G[nom]) == "function" then
                    local ok = pcall(_G[nom], "touche B")
                    print("[kronax]   " .. nom .. " : "
                        .. (ok and "arrete" or "erreur"))
                end
            end
            _G.AutoStartDone = nil   -- le prochain lancement refera le depart
        end)
end
