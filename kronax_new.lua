...
do
    --  On coupe d'abord tout ce qui tourne encore d'un chargement
    --  precedent. Sans ca, l'ancienne instance survit au relancement et
    --  deux exemplaires cliquent et se deplacent en meme temps.
    for _, nom in ipairs({"StopAutoStart", "StopAutoRetry",
        "StopCurrentFarm"}) do
        if typeof(_G[nom]) == "function" then
            pcall(_G[nom], "remplace")
        end
    end
    task.wait(0.2)
